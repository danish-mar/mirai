...
### Fixed
- **Provider Error Diagnostics**: `searchAnime` and the underlying fetch helper in `lib/anidb.ts` now log the actual HTTP status/response body (or network error) when a request to anidb.app fails, instead of a generic "Failed to search anime from provider." This is needed to diagnose provider requests failing only in some production/hosted environments (e.g. Cloudflare challenging datacenter IPs) but not locally.
